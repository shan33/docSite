...
### 理解
主要使用组件的**observer**属性
1. 组件create创建引入的时候会初始化watchInfo内容，遍历watch字段初始化watchInfo.watchCurVal的内容（主要是watch字段下的属性值）
2. 组件attached进入组件dom的时候会初始化computed的值，遍历computed字段并初始化watchInfo.computedRelatedPathValues（每个属性有关的依赖，依靠proxy（data)）；为每个计算属性添加updater方法至watchInfo.computedUpdates中；
3. 含有computed计算属性，将深度监听data，setData触发时候，会.some调用watchInfo.computedUpdates去自动更新有关的依赖，并重新计算更新data中的计算属性；
4. 含有watch会对watch字段下的每一个data属性进行监听，有变动的时候会对比watchInfo.watchCurVal以及this.data中的curVal，及时更新缓存以及传递现有数据至有关的handler；
5. 在组件的observers监听中加入新加的observe内容；
